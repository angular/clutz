...
/**
 * There are many "platform" symbols, where Closure defines them in an externs file
 * and TypeScript defines them in a lib.d.ts file.  When a Closure file references
 * a platform type like HTMLElement or platform global like encodeURIComponent,
 * rather than referring to a type that Clutz derives from the externs we'd rather
 * it refer directly to the TypeScript type.
 *
 * Note that doing this is not strictly correct (the two may have slightly different
 * definitions) but this is likely what users expect and it reduces the output size.
 *
 * This class maintains all the metadata about these types; for example, Closure
 * and TypeScript have different names for the interface that is the 'style' property
 * on an element, so this class has a map from Closure name to TypeScript name.
 */
public class PlatformSymbols {
  /**
   * Symbols that are mapped to a different specific name.
   * E.g. on the JS side when it says "CSSProperties", it's referring to the TypeScript type
   * "CSSStyleDeclaration".
   */
  ...
  /**
   * Set of all symbols that are defined in a platform externs.js that we don't have matching
   * TypeScript type for.  We could in theory just let Clutz generate types for these, but in
   * practice no users have needed them yet and it saves file space.  It should be safe to
  ...
  /**
   * Set of all symbols exposed by TypeScript's lib.*.d.ts.
   */
